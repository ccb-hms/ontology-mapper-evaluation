...
def get_mapping_counts(mappings_df, ontology_iri,
    ...
    for term in ontology.classes():
        if not any([iri_bit in term.iri for iri_bit in ontology_term_blocklist]):
            ...
            term_label = ""
            if len(term.label) > 0:
                term_label = term.label[0]
            output.append((term.iri, term_label, direct_mappings_count, inherited_mappings_count))
    output_df = pd.DataFrame(data=output, columns=['IRI', 'Label', 'Direct', 'Inherited'])
...
def _create_instance(ontology, ontology_term_iri, source_term, source_term_id, source_term_secondary_id):
    ...
    if ontology_term is not None:
        ...
        if not pd.isna(ontology_term_iri) and "," in ontology_term_iri:
            print(
                f"...warning: Multiple ontology term mappings for '{source_term}' ({source_term_id}): {ontology_term_iri}")
...
if __name__ == "__main__":
    df = get_mapping_counts(mappings_df=pd.read_csv(os.path.join(".", "output", "mappings_gwascatalog.tsv"), sep="\t"),
                            ontology_iri="http://www.ebi.ac.uk/efo/releases/v3.62.0/efo.owl",
                            ...
                            save_ontology=True)
    df.to_csv(os.path.join(".", "output", "mappings_counts_v3.tsv"), sep="\t", index=False)
